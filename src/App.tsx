...
import type { AvgByProductParams, AvgByProduct } from './api/emissions/getAvgByProduct'
...
const App = () => {
  ...
  const [avgEmissions, setAvgEmissions]: ([Array<AvgByProduct>, Function]) = useState([])
  const [isLoading, setIsLoading]: ([boolean, Function]) = useState(true)
  const [searchPanelData, setSearchPanelData]: ([SearchPanelData, Function]) = useState(initialSearchPanelData)
  ...
  useEffect(() => { 
    const loadAvgEmissions = async () => {
      ...
      if(searchPanelData.product) {
        const data: Array<AvgByProduct> = await getAvgByProduct(getAvgByProductParams)
  ...
  const handleSelectOnChange = (e: ChangeEvent<any>): void => setSearchPanelData({...searchPanelData, [e.target.name]: e.target.value })
  const handleDatePickerOnChange = (moment: Moment, name: string) => setSearchPanelData({...searchPanelData, [name]: moment })
  ...
  return (
    <StyledApp>
      ...
      <Flex grow="1" alignItems="center" justifyContent="center">
        {isLoading ? <CircularProgress size={100} /> : <EmissionsChart data={avgEmissions} />}
      </Flex>
    </StyledApp>
  );
}
